...
export default function Gallery({ onInquireAboutArtwork }: GalleryProps) {
  const [filter, setFilter] = useState<'All' | 'Oil' | 'Watercolor' | 'Mixed Media'>('All');
  const [selectedArtwork, setSelectedArtwork] = useState<Artwork | null>(null);

  const categories = ['All', 'Oil', 'Watercolor', 'Mixed Media'] as const;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8 mb-12">
          <div className="space-y-3">
            <span className="font-mono text-xs text-[#C5A47E] tracking-widest uppercase block">CURATED COLLECTION</span>
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center text-[10px] font-mono text-white/40 tracking-wider">
                      <span>C.MOREAU &mdash; {sequenceNum}</span>
                      <span>{art.year}</span>
                    </div>
          ...
          {selectedArtwork && (
            ...
            >
              ...
              >
                ...
                {/* Left side: Artwork Image displaying detail */}
                <div className="md:w-3/5 bg-[#080808] flex items-center justify-center border-b md:border-b-0 md:border-r border-white/10 relative group min-h-[300px] md:min-h-0">
                  ...
                  <img
                    ...
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute bottom-3 left-3 bg-black/90 backdrop-blur-xs text-[9px] font-mono tracking-widest uppercase text-white/95 px-3 py-1 border border-white/10">
                    Provenance Certified Original
                  </div>
                </div>
                ...
                <div className="md:w-2/5 p-8 flex flex-col justify-between space-y-8 bg-[#090909]">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3 font-mono text-xs border-y border-white/10 py-4">
                      <div className="flex justify-between">
                        ...
                        <span className="text-[#F2F2F2] text-right max-w-[200px] truncate-3-lines">{selectedArtwork.medium}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    {/* Editorial Curatorial narrative */}
                    <div className="space-y-2">
                      <h4 className="font-mono text-[10px] uppercase text-[#C5A47E] tracking-wider font-semibold">Curator Notes</h4>
                  ...
                  <div className="pt-6 border-t border-white/10 flex flex-col gap-3">
                    <div className="flex justify-between items-baseline mb-2">
                      <span className="text-xs font-mono text-neutral-500">ACQUISITION PRICE</span>
                    ...
                    {selectedArtwork.status === 'Inquire' ? (
                      ...
                      >
                        ...
                        Initiate Acquisition Consultation
                      </button>
                    ) : (
                      <div className="text-center py-2 border border-white/5 bg-white/5 text-[10px] font-mono tracking-widest uppercase text-neutral-400 rounded-xs">
                        {selectedArtwork.status === 'Sold' ? 'Commission Inquiry Only' : 'Private Holding'}
                      </div>
                    )}
                  </div>
                </div>
